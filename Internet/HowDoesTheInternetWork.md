...
## TCP/IPを利用したネットワーク
### TCPとIPとは？
TCP(Transmission Control Protocol)とはトランスポート層の送信データの信頼性を保証する通信プロトコル。
コネクション型通信（通信開始前に相手との間で仮想的な通信路を確保する）を行う。
パケットの順序や送信エラー時の再送などを制御することで、送受信するデータを保証する。

IP(Internet Protocol)とは経路制御を行い、ネットワーク間のパケット送信を制御する通信プロトコル。
IPによって構成されるネットワークではコンピュータやネットワーク機器を識別するためにIPアドレスで管理している。
コネクションレス型通信通信開始前に相手との間で、事前のやり取りなし）であるため、通信品質の保証はTCPやUPDなどの上位層に任せている。

### IPアドレスとは？
IPアドレスとはめっちゃ簡単にいうとネットワーク上での住所のこと。
個々のネットワーク機器を識別するために使うものなので、 一意の番号が割り振られている。

IPアドレスは32ビットの数字で表されているが、分かりにくいので実際には8ビットずつ区切った数字をそれぞれ１０進数で表し、「．」で区切っている。
...
- MACアドレス：LAN内の通信をするためのアドレス
- IPアドレス：ネットワーク間を通信するためのアドレス

<!-- 正確な話を書き込む -->

ハブで複数の端末が接続されているネットワークはイーサネット規格で構成されている。
イーサネットはOSI参照モデルの第1、2層をサポートするためのものでそれぞれの端末はMACアドレスで識別される。

これでコンピュータとルータ間は通信できるが、ルータとルータ間は通信できない。

ここで第3層のIP出番。
IPアドレスによってルータを識別してネットワーク間を中継する。

ネットワークを超える相手にパケットを送信する場合、送り元には自分のIPアドレス、宛先には相手のIPアドレスを記載してパケットにくっつける。
実際にデータを送るのは第2層以下のイーサネットが担当するので、先ほどのパケットにさらに送信元のMACアドレスと宛先のMACアドレスを記載する。
こうすることで、出口のルータは宛先IPアドレスを確認することで外側に中継することを認識することができる。

このパケットを受け取ったルータは、送信元を自身のMACアドレスにして、宛先を中継先のMACアドレスに書き換えイーサネットフレームとして流す。
これを繰り返して届けたい端末に情報を送り届けることができる。
<!-- ### DHCPについて
### NATとIPマスカレード -->
